...
def dashboard():
    # Header with logout
    col1, col2 = st.columns((3,1))   # ✅ fixed here
    with col1:
    ...
    # 🎥 OpenCV video feed (fixed)
    st.subheader("🎥 Traffic Camera Feed (via OpenCV)")
    video_file = "855848-hd_1920_1080_30fps.mp4"

    cap = cv2.VideoCapture(video_file)
    stframe = st.empty()

    play = st.checkbox("▶️ Play Video")
    if play:
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
